...
$page = new admin_settingpage($themename.'_homepage_academics2', get_string('academicssection', 'theme_klassplace').' 2');

/******************** Academics setting ********************/

$name = $themename.'_academics2';
$heading = get_string('academicssettingssub', 'theme_klassplace').' 2';
$information = get_string('academicssettingssub_desc', 'theme_klassplace');
$setting = new admin_setting_heading($name, $heading, $information);
$page->add($setting);

// academicstagline.
$name = $themename.'/academicstagline2';
...
// academicsheading.
$name = $themename.'/academicsheading2';
...
$name = $themename.'/academicsdescription2';
...
// usenum
$name = $themename.'/academicsusenums2';
...
// academicsimage.
$name = $themename.'/academicsimage2';
$title = get_string('academicsimage','theme_klassplace');
$description = get_string('academicsimage_desc', 'theme_klassplace');
$setting = new admin_setting_configstoredfile($name, $title, $description, 'academicsimage2');
$setting->set_updatedcallback('theme_reset_all_caches');
$page->add($setting);
...
for ($i = 1; $i <= 6; $i++) {
    ...
    $name = 'academics2_'.$i.'hdr';
    $setting = new admin_setting_heading($name, '<hr>', '');
    $page->add($setting);

    // academicslist<n>.
    $name = $themename.'/academicslist2_'.$i;
    ...
    // academicslisturl<n>.
    $name = $themename.'/academicslisturl2_'.$i;
    ...
    if (is_dir($CFG->dirroot.'/local/moodlescript')) {
        ...
        local_moodlescript_add_display_condition($page, $themename.'/academicsdispcondition2_'.$i, $i);
    }
}
