...
var custom_img = document.querySelector('.headerbkg .withimage, .headerbkg .default, .headerbkg .customimage').style.backgroundImage;
                ...
                for (var j = 0; j < mutations[i].addedNodes.length; j++) {
                    if (mutations[i].addedNodes[j].querySelectorAll) {
                        ...
                        for (var k = 0; k < card_img.length; k++) {
                            if (card_img[k] && card_img[k].style && card_img[k].style.backgroundImage.startsWith(moodle_default_img)) {
